...
--   5. DB-level metal-only gate, both directions:
...
--          reference that product; the error names how many are attached.
...
--
-- Both new/rewritten trigger functions that check sibling-table state
...
-- finishes attached. Only fires when material_id actually changes; a
-- product with zero attached finishes can change material freely.
...
begin
  ...
  if new.material_id is null then
    raise exception 'product % has % finish(es) attached — material cannot be cleared', new.id, v_finish_count;
  end if;
  ...
  if v_is_metal is not true then
    raise exception 'product % has % finish(es) attached — material must remain metal', new.id, v_finish_count;
  end if;
...
begin
  foreach t in array array[
    ...
    execute format($f$
      ...
      )$f$, 'Authed read '||t||' of house + own brand + editor products', t, t);
