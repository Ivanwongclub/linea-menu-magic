...
- `public-taxonomy.mjs` — M4 Step 1: anonymous `/products` reads families/categories from the database (sidebar, mega-menu, chips, 繁體 names); family, category and segment filters resolve through it and the listing count matches the database; draft and archived products never appear; an unknown family yields nothing; Designer Studio chips and Featured Trims use the same source.
