...
- `public-taxonomy.mjs` — M4 Steps 1 + 3: anonymous `/products` reads families/categories from the database (sidebar, mega-menu, chips, 繁體 names); empty categories and families are hidden and counts are real published counts; the Segments block exists only with more than one segment; a category narrows within its family and a category elsewhere releases the family; the listing count matches the database; draft and archived products never appear; an unknown family yields nothing; Designer Studio chips and Featured Trims use the same source.
- `home-links.mjs` — M4 Step 5: footer, home category tiles and hero CTAs link to `?family=`/`?category=` filters on the database taxonomy; an empty family is omitted from the footer and falls back to the full listing elsewhere; no hash links remain; following a link lands on the database count.
- `finish-facets.mjs` — M4 Step 4: the finish facet rail on `/products` appears only while the category scope has metal products; OR within an axis, AND across; a product matches if any attached finish matches; selection lives in the URL as `finish=axis:CODE`; chips remove it; a non-metal scope hides the rail.
- `sidebar-layout.mjs` — M4 close-out: the `/products` sidebar is sticky and bounded to the viewport with its own scroll; measured — with the column scrolled to its bottom the finish rail sits inside the viewport and the page has not moved; with many categories families collapse by default with product counts, and the family in play stays open.
- `product-detail-data.mjs` — M5 item 1: the single query `useProduct` issues, checked as an anonymous viewer on the real detail page. A fully populated product, a bare one and a non-metal one with colours; typed specification columns, typed material, attachment, size variants, colours, compliance and finishes all arrive; a finish that is not public comes back as a null embed and is dropped; an unknown slug and a draft product are "not found", not an error.
- `legacy-overrides.mjs` — M5 item 2: a deliberately wrong `name_en`/`description_en` is planted in the database and never renders; the storefront reads `name`/`name_zh_hant`/`name_zh_hans` through `localizedName` on the detail page and the cards, search matches the Chinese name, and saving in the CMS clears both overrides while leaving the trilingual columns untouched.
- `product-gallery.mjs` — M5 item 3: the hero opens on the image flagged primary while the strip follows sort_order; editor `alt_text` is used and generated when absent; the lightbox counts, navigates, zooms and closes on Escape; a single image renders no thumbnail strip; a product with no images gets a typographic empty state carrying its category, item code and a sample offer, in the interface language; no bundled stock photograph reaches the page.
- `product-specs.mjs` — M5 item 4: the typed columns drive the specification table in reading order, values are formatted (days, MOQ unit, lead-time range, booleans as words), a JSON blob planted on the row never renders, `logo_customisable` is never shown, both compliance systems appear labelled apart, the Overview block carries only item code / MOQ / lead time, a product with nothing recorded shows no table at all, the material line falls back to the legacy map, and every label translates.
- `product-sizes.mjs` — M5 item 5: the size control opens on the variant flagged default rather than the first, weight and thickness follow the selection, ligne shows alongside millimetres on round sizes and is suppressed on labelled non-round hardware, a single variant still renders and a product with none renders no control; also asserts the whole detail page, not just the spec table, speaks the interface language.
- `product-colour-finish.mjs` — M5 item 6: a metal product shows its attached public finishes as swatches from the shared finishes module, opening on the default rather than the first, with the private one withheld and its colour rows ignored; selecting a finish changes the name and CYC code and leaves the hero image untouched; a non-metal product shows colour chips coloured by their hex; a product that is neither renders no section at all; both carry the indicative-swatch line and both translate.
- `brand-scoped-product.mjs` — M5 item 7: a product with `brand_id` set is readable only by that brand's members and by catalogue editors, checked at the API boundary and then in the page; an anonymous visitor and a signed-in outsider get the not-found page with no name, item code, spec table or gallery leaking, and no error; the member and the editor see the product normally.
